...
import { InsuranceBuyerServices } from '../Services/insurance-buyer-services';
...
@Component({
  ...
  imports: [CommonModule, ShowButton, Spin],
  templateUrl: './section-info.html',
  styleUrls: ['./section-info.css'],
  providers:[InsuranceBuyerServices]
})
export class SectionInfo implements AfterViewInit {
  ...
  constructor(
    ...
    private stripe: StripeComponenteDownload
  ) {}
  ...
  async buyInsurance(insurancesName:string){
    this.spin.show()
    await this.stripe.checkout(insurancesName, this.apiUrl)
    this.spin.hide()
  }
}
