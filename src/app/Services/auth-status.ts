...
export class AuthStatus {
  constructor(private storageService: BrowserStorageServices, private writeEmail: WriteEmailInRDS, private route:Router) {}

  private isAuthenticatedSubject = new BehaviorSubject<boolean>(this.checkAuth());
  public isAuthenticated$ = this.isAuthenticatedSubject.asObservable();

  private checkAuth(): boolean {
    const email = this.storageService.getLocalItem('email') || 
                  this.storageService.getSessionItem('email');
    const expiresAt = this.storageService.getLocalItem('expiresAt') || 
                      this.storageService.getSessionItem('expiresAt');
  ...
  private saveAuthData(email: string, rememberMe: boolean): void {
    ...
    if (rememberMe) {
      ...
      localStorage.setItem('expiresAt', String(Date.now() + 60 * 60 * 1000)); // 30 minutos
    } else {
